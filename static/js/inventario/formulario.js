    ...
    function precioTotalAgregar(origen) {
      ...
      function setPriceDatatable(){
        ...
        tabla = $('#tabla-precios-tmp').DataTable({
            ...
            serverSide: true,
            ajax:{
                url: '../servidor/inventario/server_processing_precios_tmp.php?tabla=precios_tmp',
                dataType: 'json'
            },
            searching: false,
            info:false,
                ...
                { data:null, title:'Opciones', render: function(row){
                    ...
                    <div class='row'>
                        <div class='col-12 col-md-12'>
                            <div class="btn btn-danger" onclick="eliminarPrecioTMP(${row[0]}, 'individual')"><i class="fa-solid fa-trash"></i></div>
                        </div>
                    </div>
                    `
                }}
      ...
      function agregarPrecioTmp(){
        ...
        if(precio_base == null || precio_base == undefined || precio_base.length == 0 || precio_base == ""){
            ...
            $.ajax({
                type: "POST",
                url: "../servidor/inventario/agregar-precios-tmp.php",
                data: {"costo": costo, "precio_base": precio_base,
                       "tasa": tasa, "etiqueta": etiqueta, "neto": neto},
                dataType: "JSON",
      ...
      function eliminarPrecioTMP(precio_tmp_id, tipo){
       
        $.ajax({
            type: "POST",
            url: "../servidor/inventario/eliminar-precios-tmp.php",
            data: {"id_precio": precio_tmp_id, "tipo": tipo},
            dataType: "JSON",
            success: function (response) {
                console.log(response);
                setPriceDatatable()
